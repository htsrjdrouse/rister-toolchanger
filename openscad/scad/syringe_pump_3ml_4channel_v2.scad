...
//translate([0,90,0])syringe_3ml_set();
...
//translate([0,70-4.45+20,-0.1])syringe3ml_multichannel_syringe3ml_clamp();

//translate([0,70+20,-0.1])syringe3ml_multichannel_syringe3ml_clamp_luerlock();
//translate([-10.5,0-4.5,0])syringe_3ml_stack_clamp();
//translate([-10,0-4.5,0])syringe_3ml_stack_clamp_partb();

// Offset to centre syringe bank symmetrically on lead screw.
// Lead screw at X≈-122. Original bank centre at X≈-148.5.
// +26.5mm shift puts syringes at -156.5, -133.5, -110.5, -87.5
// i.e. ±11.5mm and ±34.5mm from lead screw — fully symmetric.
// Adjust SYRINGE_OFFSET if lead screw position differs.
SYRINGE_OFFSET = 26.5;

module four_channel_3ml_syringe_assy(){

// ── Syringe bank — shifts with SYRINGE_OFFSET ─────────────────
translate([SYRINGE_OFFSET,0,0]){
    translate([0,70-4.45+20,-0.1])tyco_multichannel_syringe3ml_clamp();
    translate([0,30-20-4.+20,0])tyco_multichannel_syringe3ml_clamp_top_luerlock();
    translate([0,70+20,-0.1])tyco_multichannel_syringe3ml_clamp_luerlock();
...
// ── Fixed parts — motor, rail, extrusion, tslot ───────────────
translate([0,90-4,0])multichannel_syringe3ml_clamp_extrusion_connect();
translate([0,90+60,0])multichannel_syringe3ml_clamp_extrusion_connect();
...
CLAMP_X0 = -183;                   // X origin (match original layout)
CLAMP_Y0 = 166;
...
module syringe_3ml_set(){
    for(i=[0:N_CHAN-1]){
        ...
        translate([-183+(i*(SPACING+3)),170,44])rotate([0,90,90])syringe_3ml();
    }
}
    ...
    difference(){
        translate([-183-13/2-5,170-13/2,30]){
            union(){
                ...
                translate([0+41.2,2+7.25-7-3-5,2]) cube([25,10+5,4]);
            }
        }
        translate([-183-13/2-5,170-13/2,30]){
    ...
    difference(){
        union(){
            translate([-183-13/2-5,170-13/2,30]){
                ...
                translate([69,2-15,0]) cube([18+8, 23+20, 5]);
            }
        }
...
module syringe3ml_multichannel_syringe3ml_clamp_top_luerlock(){
    difference(){
        union(){
            translate([-183-13/2-5,170-13/2,30]){
                translate([0-8,2+7.25+60-1,10/2+10])
                    cube([N_CHAN*SPACING+SPACING+6, 13/2+1, 15/2+2-3]);
            }
        }
        for(i=[0:N_CHAN-1]){
            #translate([-183+(i*(SPACING+3)),170-50+50,44])
                rotate([0,90,90]) cylinder(r=BARREL_SLOT/2, h=100);
        }
	for(i=[0:4]){
        translate([-77.5-(N_CHAN*SPACING)-1.7-36.3+(i*23), 175.6+60-0.4, 0]) cylinder(r=3.7/2,h=150, $fn=100);
	}
    }
}
    ...
    difference(){
        union(){
            translate([-183-13/2-5,170-13/2+60,30]){
                ...
                translate([69,2-15-1,0]) cube([18+8, 23+20+1, 5]);
            }
        }
        ...
        for(i=[0:N_CHAN-1]){
           ...
           #translate([-183+(i*(SPACING+3)),170-0,44])
                 rotate([0,90,90]) cylinder(r=BARREL_SLOT/2, h=100);
