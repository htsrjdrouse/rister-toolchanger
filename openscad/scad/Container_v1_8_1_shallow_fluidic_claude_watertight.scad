...
intersection()
{
	...
	union()
	{
		...
		// Left side barb hole - stepped design for single-ridge barb sealing
		translate([-20+9.4,0,1.2-1])rotate([0,90,0]){
			// Funnel entry for easy barb insertion
			#cylinder(d1=2.0, d2=1.35, h=2, $fn=50);
			// TIGHT grip zone (compresses single barb ridge)
			translate([0,0,2])
			#cylinder(d=1.35, h=3, $fn=50);
			// Exit channel (looser for fluid flow)
			translate([0,0,5])
			#cylinder(d=1.4, h=15, $fn=50);
		}
		
		// Internal chamber connection (widened for bubble removal)
		hull(){
		...
		// Right side barb hole - stepped design for single-ridge barb sealing
		translate([4-0.4,0,1.2-1])rotate([0,90,0]){
			// Exit channel from inside (looser)
			#cylinder(d=1.50, h=2, $fn=50);
			// TIGHT grip zone (compresses single barb ridge)
			translate([0,0,2])
			#cylinder(d=1.35, h=3, $fn=50);
			// Funnel entry for easy barb insertion
			translate([0,0,5])
			#cylinder(d1=1.35, d2=2.0, h=2, $fn=50);
			// External extension
			translate([0,0,7])
			#cylinder(d=2.0, h=13, $fn=50);
		}
		}
		else
