...
// ============================================================
// 4-Channel 3ml Syringe Pump Assembly
// Modified from 8-channel 1ml version
// Key parameter changes:
//   - Channels: 8 → 4
//   - Channel spacing: 14mm → 20mm
//   - Barrel OD: 6.5mm → 8.9mm  (standard 3ml BD/generic)
//   - Barrel ID: 5.0mm → 7.3mm
//   - Plunger head dia: 4.8mm → 7.5mm
//   - Barrel length: 73mm → 95mm
//   - Total syringe length: 155mm → 180mm
//   - Flange width: 12mm → 15mm
//   - Flange length: 19mm → 22mm
//   - Luer lock collar OD: 9mm → 11mm
// ============================================================

// --- Global syringe parameters (edit here to tune fit) ---
SYR_BARREL_OD     = 10.9;    // 3ml barrel outer diameter mm
SYR_BARREL_ID     = 7.3;    // 3ml barrel inner diameter mm
SYR_BARREL_LENGTH = 95;     // active barrel length mm
SYR_TOTAL_LENGTH  = 180;    // tip-to-thumb-rest mm
SYR_PLUNGER_DIA   = 7.5;    // plunger head/seal diameter mm
SYR_FLANGE_W      = 15;     // finger flange width mm
SYR_FLANGE_L      = 22;     // finger flange length mm
SYR_FLANGE_T      = 2.2;    // finger flange thickness mm
SYR_LUER_OD       = 12;     // luer lock collar OD mm
SYR_CLAMP_CLEAR   = 0.2;    // diametric clearance for clamp slots

N_CHAN   = 4;      // number of channels
SPACING  = 20;     // channel pitch mm (was 14mm for 1ml)
//SYRINGE_OFFSET = 25.7; // centres syringe bank on lead screw (X=-122.8)
SYRINGE_OFFSET = -11.5; // centres syringe bank on lead screw (X=-122.8)

//four_channel_3ml_syringe_assy();

translate([-10.5,0-4.5,0])syringe_3ml_stack_clamp();


module four_channel_3ml_syringe_assy(){
translate([0,4.5-4.5-4.5,0])translate([0,90,0])syringe_3ml_set();

translate([2,70-4.45+20,-0.1])syringe3ml_multichannel_syringe3ml_clamp();
translate([0,30-20-4.+20,0])syringe3ml_multichannel_syringe3ml_clamp_top_luerlock();
translate([2,70+20,-0.1])syringe3ml_multichannel_syringe3ml_clamp_luerlock();

translate([0,-4-4.5,0])multichannel_plunger_clamp_3ml();
#translate([-10.5,0-4.5,0])syringe_3ml_stack_clamp();
translate([-10,0-4.5,0])syringe_3ml_stack_clamp_partb();
translate([-121.8-1,54.1+3-4.5,3])rotate([0,0,0])oneml_syringe_stepper_linear_m8nut_coupler();
translate([-100,160-4.5,0])translate([-25.75,123+3,-30])rotate([90,-90,0])multichannel_syringeshuttle_clipbracket();
translate([-100,176-4.5,0])iverntech_pump_slider_plate();

translate([2,90-4,0])multichannel_syringe3ml_clamp_extrusion_connect();
translate([2,90+60,0])multichannel_syringe3ml_clamp_extrusion_connect();
...
// ============================================================
// 3ml 4-channel barrel clamp parts
// Barrel slot dia = SYR_BARREL_OD + SYR_CLAMP_CLEAR
// Luer slot dia   = SYR_LUER_OD  + SYR_CLAMP_CLEAR
// Channel pitch   = SPACING (20mm)
// Total rail width = N_CHAN * SPACING + some margin → ~90mm
// ============================================================

RAIL_W = N_CHAN * SPACING + 10;   // 90mm total clamp width
CLAMP_X0 = -183 + SYRINGE_OFFSET;                   // X origin (match original layout)
CLAMP_Y0 = 166;
BARREL_SLOT = SYR_BARREL_OD + SYR_CLAMP_CLEAR;
LUER_SLOT   = SYR_LUER_OD   + SYR_CLAMP_CLEAR;

// Part A — lower barrel clamp body
module syringe_3ml_stack_clamp(){
    difference(){
        union(){
            translate([CLAMP_X0-7, CLAMP_Y0, 36])             cube([RAIL_W+12, 23, 4]);
            translate([CLAMP_X0-7, CLAMP_Y0+18.3, 36])        cube([RAIL_W+0.5+12, 5, 16]);
        }
        for(i=[0:N_CHAN-1]){
            // M3 screw hole at base
            translate([CLAMP_X0+10.5+i*SPACING, CLAMP_Y0+7.3, 36-5])
                cylinder(d=3.7, h=20, $fn=60);
            // M3 side screw
            //translate([CLAMP_X0+10.5+(i*SPACING+20)-10, CLAMP_Y0+7.3+25, 46])
            //    rotate([90,0,0]) cylinder(d=3.7, h=20, $fn=60);
        }
            translate([CLAMP_X0+10.5+(0*SPACING+20)-9-23, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=3.7, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)-9, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=3.7, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)+14.5, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=3.7, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)+14.5+23, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=3.7, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)+14.5+46, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=3.7, h=20, $fn=60);
    }
}

// Part B — upper luer-lock clamp cap
module syringe_3ml_stack_clamp_partb(){
    difference(){
        union(){
            translate([CLAMP_X0-7.6, CLAMP_Y0+23.4, 39.2-3]) cube([RAIL_W+0.8+11.7, 6.5, 16]);
        }
        for(i=[0:N_CHAN-1]){
            // Barrel slot (oval cutout for barrel body)

        }

	   for(i=[0:3]){
	    translate([-0.7+(i*23),-10.1,0])hull(){
            translate([CLAMP_X0+10.5+(0*SPACING), CLAMP_Y0+35.2-0.4, 53])rotate([90,0,0]) cylinder(d=LUER_SLOT, h=1.3, $fn=80);
            translate([CLAMP_X0+10.5+(0*SPACING), CLAMP_Y0+35.2-0.4, 44])rotate([90,0,0]) cylinder(d=LUER_SLOT, h=1.3, $fn=80);
	    }
	    translate([-0.7+(i*23),-2.6-2,0])hull(){
            translate([CLAMP_X0+10.5+(0*SPACING), CLAMP_Y0+35.3, 53])rotate([90,0,0]) cylinder(d=9, h=7.7, $fn=80);
            translate([CLAMP_X0+10.5+(0*SPACING), CLAMP_Y0+35.3, 44])rotate([90,0,0]) cylinder(d=9, h=7.7, $fn=80);
	    }
	   }

            translate([CLAMP_X0+10.5+(0*SPACING+20)-9-23, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=2.8, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)-9, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=2.8, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)+14.5, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=2.8, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)+14.5+23, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=2.8, h=20, $fn=60);
            translate([CLAMP_X0+10.5+(0*SPACING+20)+14.5+46, CLAMP_Y0+7.3+25, 46])rotate([90,0,0]) cylinder(d=2.8, h=20, $fn=60);
...
// --- 4-channel 3ml syringe set ---
module syringe_3ml_set(){
    for(i=[0:N_CHAN-1]){
        //color("")translate([-85-(i*SPACING),105.5,44])rotate([-0,0,-90])syringe_3ml_plungerclip();
        translate([-183+SYRINGE_OFFSET+(i*(SPACING+3)),170,44])rotate([0,90,90])syringe_3ml();
    }
}

// Plunger clip scaled for 3ml plunger thumb rest width (~18mm)
module syringe_3ml_plungerclip(){
    difference(){
        union(){
            translate([0-4+10,-5+2.5,-8])cube([5,10+5,16]);
            translate([0-4+10,-5+2.5,-8])cube([14+15-10+4+4,10+5-1,4]);
        }
        translate([18+4+4,0,-8])cylinder(r=3.7/2,h=40);
        translate([2.5-2,40,4])rotate([90,0,0])cylinder(r=2.8/2,h=100);
    }
}
...
module multichannel_plunger_clamp_3ml(){
    difference(){
        translate([-183+SYRINGE_OFFSET-13/2-5,170-13/2,30]){
            union(){
                translate([0+1,2+7.25-2,2]) cube([N_CHAN*SPACING+SPACING-10, 13/2+7, 4]);
                translate([0+41.2+11,2+7.25-7-3-5,2]) cube([25,10+5,4]);
            }
        }
        translate([-183+SYRINGE_OFFSET-13/2-5,170-13/2,30]){
	    translate([11.5,0,0]){
            #translate([64.7-5-36.15,14.4-3-0.87-50+8-0.08-5,-55.75])
                translate([17.25,5.8,-10])translate([-9+15-2,30,23])
                rotate([0,0,0])cylinder(r=3.7/2, h=200,$fn=100);
            #translate([64.7-5-36.15+17.5,14.4-3-0.87-50+8-0.08-5,-55.75])
                translate([17.25,5.8,-10])translate([-9+15-2,30,23])
                rotate([0,0,0])cylinder(r=3.7/2, h=200, $fn=100);
	    }
            for(i=[0:N_CHAN-1]){
                translate([64.7-5-36.15+17.5-50.75+(i*SPACING),14.4-3-0.87-50+8-0.08+9.75,-55.75])
                    translate([17.25,5.8,-10])translate([-9+15-2,30,23])
                    rotate([0,0,0])cylinder(r=2.8/2, h=200,$fn=100);
            }
        }
    }
}

module multichannel_syringe3ml_clamp_extrusion_connect(){
    difference(){
        union(){
            translate([-183+SYRINGE_OFFSET-13/2-5+8,170-13/2,30]){
                translate([69,2-15,-9.5])cube([18+8,23+20,9.48]);
            }
        }
        translate([-3,1,0]){
...
tyco_four_multichannel_assay();
translate([-100,160,0])nextgen_syringe1ml_multichannel_assy();
multichannel_plunger_clamp_3ml();
translate([0,12,0]){
translate([-100,160,0]) 
...
module tyco_four_multichannel_assay(){
    translate([0,20,0]){
        difference(){
            union(){
		difference(){	
                translate([0,70-4.45,-0.1])tyco_multichannel_syringe3ml_clamp();
                //translate([-150,170,-0.1])cube([150,150,50]);
		}
                translate([0,30-20-4.,0])tyco_multichannel_syringe3ml_clamp_top_luerlock();
                translate([0,70,-0.1])tyco_multichannel_syringe3ml_clamp_luerlock();
            }
        }
    }
}

module syringe3ml_multichannel_syringe3ml_clamp(){
    difference(){
        union(){
            translate([-183+SYRINGE_OFFSET-13/2-5,170-13/2,30]){
                translate([-8,2,0]) cube([N_CHAN*(SPACING)+SPACING+6, 13.9+2, 15]);
                translate([69+8,2-15+0.5,0]) cube([18+8, 23+20, 5]);
            }
        }
        ...
        translate([-120,220-60,32]) cylinder(r=9.7/2,h=14);
        translate([-120+15,220-60,32]) cylinder(r=9.7/2,h=14);
        translate([-120+7.5,220-60+23,32]) cylinder(r=9.7/2,h=14);
        }
        for(i=[0:N_CHAN-1]){
            translate([-183+SYRINGE_OFFSET+(i*(SPACING+3)-2),170,44])
                rotate([0,90,90]) syringe_3ml();
            translate([-183+SYRINGE_OFFSET+(i*(SPACING+3)-2),170,44])
                rotate([0,90,90]) cylinder(r=BARREL_SLOT/2, h=100);
            translate([-183+SYRINGE_OFFSET+(i*(SPACING+3)-2),170-10,44])
                rotate([0,90,90]) cylinder(d=9, h=10);


            translate([-126-(i*(SPACING+3))-(i>0?0.5:0), 175.6, 0])
                cylinder(r=2.8/2, h=150, $fn=100);
        }
            translate([-126-(-1*(SPACING+3))-(i>0?0.5:0), 175.6, 0])
                cylinder(r=2.8/2, h=150, $fn=100);
        //translate([-77.5-(N_CHAN*SPACING)-1.7, 175.6, 0]) cylinder(r=2.8/2,h=150);
	translate([-2,-59.5,0])for(i=[0:4]){
        translate([-77.5-(N_CHAN*SPACING)-1.7-36.3+(i*23)-10.5, 175.6+60-0.4, 0]) cylinder(r=2.8/2,h=150, $fn=100);
	}
    }
}

module syringe3ml_multichannel_syringe3ml_clamp_top_luerlock(){
    difference(){
        union(){
            translate([-183+SYRINGE_OFFSET-13/2-5,170-13/2,30]){
                translate([0-8+2,2+7.25+60-1,10/2+10])
                    cube([N_CHAN*SPACING+SPACING+6, 13/2+1, 15/2+2-3]);
            }
        }
        for(i=[0:N_CHAN-1]){
            translate([-183+SYRINGE_OFFSET+(i*(SPACING+3)),170-50+50,44])
                rotate([0,90,90]) cylinder(r=BARREL_SLOT/2, h=100);
        }
	for(i=[0:4]){
        translate([-77.5-(N_CHAN*SPACING)-1.7-36.3+(i*23)-10.5, 175.6+60-0.4, 0]) cylinder(r=3.7/2,h=150, $fn=100);
	}
    }
}

// Legacy BD syringe_1ml kept for reference only
module syringe_1ml(){
union(){
...
module syringe3ml_multichannel_syringe3ml_clamp_luerlock(){
    difference(){
        union(){
            translate([-183+SYRINGE_OFFSET-13/2-5,170-13/2+60,30]){
                translate([0,2+6.9,0]) cube([N_CHAN*SPACING+SPACING, 13.9-7, 15]);
                translate([69+8,2-15,0]) cube([18+8, 23+20, 5]);
            }
        }
        translate([-3,1,0]){
        ...
        translate([-120,220-60+60,32]) cylinder(r=9.7/2,h=10);
        translate([-120+15,220-60+60,32]) cylinder(r=9.7/2,h=10);
        translate([-120+7.5,220-60+23+60,32]) cylinder(r=9.7/2,h=10);
	}

        for(i=[0:N_CHAN-1]){
            //translate([-183+(i*SPACING),170-50,44])
            //    rotate([0,90,90]) cylinder(r=BARREL_SLOT/2, h=200);
           translate([-183+SYRINGE_OFFSET+(i*(SPACING+3)-2),170-0,44])
                 rotate([0,90,90]) cylinder(r=BARREL_SLOT/2, h=100);
...
// --- Legacy 1ml model kept for reference (not used in assembly) ---
module tuberculin_syringe_1ml(
    ...
    barrel_length = 73;
    luer_length = 2; luer_od = 4; luer_taper_length = 6;
    color("lightblue")cylinder(d=3,h=130,$fn=30);
    color("white", 0.7) {
        difference() {
            cylinder(h=barrel_length, d=barrel_od, $fn=32);
            translate([0,0,-0.1]) cylinder(h=barrel_length+0.2, d=barrel_id, $fn=32);
        }
        translate([0,0,barrel_length]) {
            cylinder(h=luer_taper_length, d1=9, d2=9, $fn=32);
            translate([0,0,luer_taper_length]) cylinder(h=luer_length, d=luer_od, $fn=32);
        }
        translate([0,0,0]) {
            difference() {
                hull() {
                    translate([-flange_length/2,-flange_width/2,0]) cube([flange_length,flange_width,flange_thick]);
                    cylinder(h=2,d=barrel_od,$fn=32);
                }
                translate([0,0,-0.1]) cylinder(h=2.2,d=barrel_id,$fn=32);
            }
        }
    }
    ...
    color("gray",0.8) {
        translate([0,0,-plunger_extension]) {
            ...
            translate([0,0,plunger_extension]) cylinder(h=8,d=plunger_head_dia,$fn=32);
            translate([0,0,-1]) cylinder(h=2,d=10.1,$fn=32);
        }
    }
}

// --- 3ml syringe model (uses global parameters) ---
module syringe_3ml(
    total_length  = SYR_TOTAL_LENGTH,
    barrel_od     = SYR_BARREL_OD,
    barrel_id     = SYR_BARREL_ID,
    barrel_length = SYR_BARREL_LENGTH,
    plunger_dia   = SYR_PLUNGER_DIA,
    flange_l      = SYR_FLANGE_L,
    flange_w      = SYR_FLANGE_W,
    flange_t      = SYR_FLANGE_T,
    luer_od       = SYR_LUER_OD
) {
    luer_taper_h = 7;
    luer_tip_h   = 3;
    ...
    // Luer lock tip
    color("lightblue") cylinder(d=3, h=140, $fn=30);

    color("white", 0.8) {
        // Barrel
        difference() {
            cylinder(h=barrel_length, d=barrel_od, $fn=36);
            translate([0,0,-0.1]) cylinder(h=barrel_length+0.2, d=barrel_id, $fn=36);
        }
        // Luer collar at tip end
        translate([0,0,barrel_length]) {
            cylinder(h=luer_taper_h, d=luer_od, $fn=36);
            translate([0,0,luer_taper_h]) cylinder(h=luer_tip_h, d=luer_tip_d, $fn=36);
        }
        // Finger flange at base
        translate([0,0,0]) {
            difference() {
                hull() {
                    translate([-flange_l/2,-flange_w/2,0]) cube([flange_l,flange_w,flange_t]);
                    cylinder(h=flange_t, d=barrel_od, $fn=36);
                }
                translate([0,0,-0.1]) cylinder(h=flange_t+0.2, d=barrel_id, $fn=36);
            }
        }
    }
    // Plunger
    plunger_ext = total_length - barrel_length - 12;
    color("gray",0.8) {
        translate([0,0,-plunger_ext]) {
            cylinder(h=plunger_ext, d=2.0, $fn=16);          // rod
            translate([0,0,plunger_ext]) cylinder(h=9, d=plunger_dia, $fn=36); // seal
            translate([0,0,-2.2+4.55]) cylinder(h=1.5, d=SYR_LUER_OD, $fn=36); // thumb rest
        }
    }
}
