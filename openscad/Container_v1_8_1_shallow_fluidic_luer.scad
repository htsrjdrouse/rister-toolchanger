...
// Version 1.8.1-shallow-fluidic-luer
...
// - Added male luer lock fittings for female luer barb adapters
...
// Enable luer lock fittings on sides
enable_luer_fittings = true;

// Height of luer fitting centers from container bottom (mm)
luer_height = 1.2;

// Distance between the two luer fittings (center to center, mm) - affects channel length
luer_spacing = 10; // [6:Tight, 8:Medium, 10:Standard, 12:Wide]

// Internal channel diameter (mm) - for fluid flow through container
internal_channel_diameter = 1.0; // [0.6:Fine, 0.8:Small, 1.0:Medium, 1.5:Large]
...
// Male Luer Lock Module
// Creates a male luer taper that accepts female luer lock fittings
module male_luer_lock(channel_d=1.0) {
    // Standard luer taper: 6% taper (ISO 594)
    // Small end: 2.5mm diameter, Large end: 3.5mm diameter, Length: ~8mm
    
    difference() {
        union() {
            // Main tapered body
            cylinder(d1=3.6, d2=2.6, h=8, $fn=50);
            
            // Small extension for extra grip
            translate([0,0,8])
            cylinder(d=2.5, h=1, $fn=50);
            
            // Base flange for strength
            cylinder(d=5, h=1.5, $fn=50);
        }
        
        // Through-hole for fluid
        translate([0,0,-0.1])
        cylinder(d=channel_d, h=10, $fn=50);
    }
}
...
intersection()
{
	...
	union()
	{
		...
		union(){
			...
			// Thicker hexagonal base for structural support
			translate([0,0,-2])
			rotate([0,0,30])
			cylinder(d=23,h=2.2+2,$fn=6);
			
			// Add male luer lock fittings if enabled
			if(enable_luer_fittings) {
				// Left side luer fitting (inlet)
				translate([-(luer_spacing/2 + 4.5), 0, luer_height])
				rotate([0,90,0])
				male_luer_lock(internal_channel_diameter);
				
				// Right side luer fitting (outlet)
				translate([luer_spacing/2 + 4.5, 0, luer_height])
				rotate([0,-90,0])
				male_luer_lock(internal_channel_diameter);
			}
		...
		if(enable_luer_fittings) {
			// Internal channel connecting the two luer fittings
			translate([0, 0, luer_height])
			rotate([0,90,0])
			cylinder(d=internal_channel_diameter, h=luer_spacing + 20, center=true, $fn=50);
		}
		}
		else
